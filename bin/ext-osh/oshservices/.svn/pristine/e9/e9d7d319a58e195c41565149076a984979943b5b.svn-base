...
/**
 * This class converts the input file to impex. Behaves differently from default hybris implementation in 2 ways . takes
 * custom field separator for each converter . takes custom lines to skip for each converter
 */
public class OshImpexConverter implements ImpexConverter
{
			...
			final StringBuilder builder)
	{
		...
		/*
		 * if (StringUtils.isBlank(colValue)) { throw new IllegalArgumentException("Missing value for custom column " +
		 * mapIdx); }
		 */
		String newColValue;
		if (colValue != null)

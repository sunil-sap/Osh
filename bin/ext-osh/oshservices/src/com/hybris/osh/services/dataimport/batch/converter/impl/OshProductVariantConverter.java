 ...
 *
 */
public class OshProductVariantConverter implements ImpexConverter
{
	...
	public String convert(final Map<Integer, String> row, final Long sequenceId)
	{
		...
		if (!MapUtils.isEmpty(row))
		{
			...
			while (idx > -1)
			{
				...
				if (checkProductNotNull(row.get(Integer.valueOf(0))))
				{
	...
	private boolean checkProductNotNull(final String code)
	{
		...
		final ProductModel product = oshProductService.getProduct(catalogVersion, "B-" + code);

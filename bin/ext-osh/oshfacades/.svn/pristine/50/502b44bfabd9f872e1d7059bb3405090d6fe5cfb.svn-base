...
/**
 * Specialised version of the DefaultCheckoutFlowFacade that allows the checkout flow and pci strategy to be
 * overridden in the session. This is primarily used for demonstration purposes and you may not need to use
 * this sub-class in your environment.
 */
public class SessionOverrideCheckoutFlowFacade extends DefaultCheckoutFlowFacade
{
	...
	{
		final CheckoutFlowEnum sessionOverride = getSessionService().getAttribute(SESSION_KEY_CHECKOUT_FLOW);
	...
	{
		final SessionService ss = getStaticSessionService();
		ss.removeAttribute(SESSION_KEY_CHECKOUT_FLOW);
		ss.removeAttribute(SESSION_KEY_SUBSCRIPTION_PCI_OPTION);
	}

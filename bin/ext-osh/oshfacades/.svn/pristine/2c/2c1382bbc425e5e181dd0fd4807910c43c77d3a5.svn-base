 ...
 *  
 */
package com.hybris.osh.facades.suggestion;
...
import com.hybris.osh.facades.suggestion.impl.DefaultSimpleSuggestionFacade;
...
public class DefaultSimpleSuggestionFacadeIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void setUp() throws Exception
	{
		...
		importCsv("/oshfacades/test/testSimpleSuggestionFacade.csv", "utf-8");
	}
	...
	public void testReferencesForPurchasedInCategory()
	{
		...
		result = simpleSuggestionFacade.getReferencesForPurchasedInCategory("cameras", ProductReferenceTypeEnum.SIMILAR, false,
				null);
		Assert.assertEquals(1, result.size());
		result = simpleSuggestionFacade.getReferencesForPurchasedInCategory("cameras", ProductReferenceTypeEnum.ACCESSORIES, false,
				null);
		Assert.assertEquals(2, result.size());
		result = simpleSuggestionFacade.getReferencesForPurchasedInCategory("cameras", ProductReferenceTypeEnum.ACCESSORIES, true,
				null);
		Assert.assertEquals(1, result.size());

 ...
 *  
 */
package com.hybris.osh.facades.storefinder.converters;

import de.hybris.platform.acceleratorservices.storefinder.data.PointOfServiceDistanceData;
...
/**
 */
public class SearchPagePointOfServiceDistanceConverter<SOURCE extends StoreFinderSearchPageData<PointOfServiceDistanceData>, TARGET extends StoreFinderSearchPageData<PointOfServiceData>>
		extends AbstractConverter<SOURCE, TARGET>
{
	...
	{
		return (TARGET) new StoreFinderSearchPageData<PointOfServiceData>();
	}
	...
	protected String getStoreUrlQueryParams(final TARGET target)
	{
		if (target.getLocationText() != null && !target.getLocationText().isEmpty())
		{
			...
			return UriComponentsBuilder.fromPath("").queryParam("lat", Double.valueOf(target.getSourceLatitude())).queryParam("long", Double.valueOf(target.getSourceLongitude())).queryParam("q", target.getLocationText()).build().toString();
		}
		// Build URL to position query
		return UriComponentsBuilder.fromPath("").queryParam("lat", Double.valueOf(target.getSourceLatitude())).queryParam("long", Double.valueOf(target.getSourceLongitude())).build().toString();
	}
}

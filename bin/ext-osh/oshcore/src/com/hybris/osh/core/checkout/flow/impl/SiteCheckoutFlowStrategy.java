...
public class SiteCheckoutFlowStrategy extends AbstractCheckoutFlowStrategy
{
	...
	public CheckoutFlowEnum getCheckoutFlow()
	{
		if (hasCurrentSite())
		{
			...
			final CheckoutFlowStrategy strategy = siteMappings.get(siteUid);
		...
		else
		{
			if (LOG.isDebugEnabled())
			{
				LOG.debug("Using default " + getDefaultStrategy() + " - no available current site ");
			}
			return getDefaultStrategy().getCheckoutFlow();
		}
	}

	protected boolean hasCurrentSite()
	{

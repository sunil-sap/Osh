...
public class NewCustomerCheckoutFlowStrategy extends AbstractCheckoutFlowStrategy
{
	...
	protected boolean isNewCustomer()
	{
		...
		{
			return haveNoDefaultAddress((CustomerModel) user) || haveNoPaymentInfo((CustomerModel) user);
		}
	...
	private boolean haveNoDefaultAddress(final CustomerModel user)
	{
		return getCustomerAccountService().getDefaultAddress(user) == null;
	}

	private boolean haveNoPaymentInfo(final CustomerModel user)
	{
		final List<CreditCardPaymentInfoModel> paymentInfo = getCustomerAccountService().getCreditCardPaymentInfos(user, true);

		return CollectionUtils.isEmpty(paymentInfo);
	}

...
import com.hybris.osh.core.service.SFTPProductExportService;
 ...
 * 
 */
public class DefaultSFTPProductExportService implements SFTPProductExportService
{
	...
	private static final String SFTP_COPY_FOLDER = "pom.product.copy.product.file.sftp.to.folder";
	 ...
	 */
	@Override
	public boolean isProductFileExported()
	{
		...
		final File storeFolder = new File(productLocalFolder);
		...
		return oshSFTPService.retriveFile(sftpHost, sftpUser, null, privateKey, productSourceFolder, toLocalFolder,
				archivePath.toString(), productLocalFolder);
	...
	public boolean getArchiveStatus()
	{
		...
		final String fileName = "catalog_";
		...
		final boolean result = oshSFTPService.archiveFoldersFile(sftpHost, sftpUser, null, privateKey, productLocalFolder,
				removeFile, archivePath.toString(), productSourceFolder);

		if (result)
		{
			final File localPricefolder = new File(productLocalFolder);
			...
			for (int i = 0; i < listOfFilesInLocalFolder.length; i++)
			{
				if (listOfFilesInLocalFolder[i].isFile() && !listOfFilesInLocalFolder[i].getName().contains("control_"))
				{
					...
					final File targetFile = new File(localFolderPath + File.separator + "catalog" + fileExt);

 ...
 * 
 */
public class DefaultSFTPOnlineExportService implements SFTPOnlineExportService
{
	private static final Logger LOG = Logger.getLogger(DefaultSFTPOnlineExportService.class);
	...
	private static final String SFTP_FOLDER = "pom.inventory.file.sftp.folder";
	private static final String SFTP_COPY_FOLDER = "pom.product.copy.inv.file.sftp.to.folder";

	public static String sftpHost = null;
	public static String sftpUser = null;
	public static String onlineInvSourceFolder = null;
	public static String onlineInvLocalFolder = null;
	 ...
	 */
	@Override
	public boolean isOnlineInventoryFileExported()
	{
		...
		onlineInvSourceFolder = configurationService.getConfiguration().getString(SFTP_FOLDER);
		Assert.notNull(onlineInvSourceFolder, "SFTP User Can Not BE NULL");
		...
		onlineInvLocalFolder = configurationService.getConfiguration().getString(SFTP_COPY_FOLDER);
		Assert.notNull(onlineInvLocalFolder, "SFTP storeLocalStatusFolder  Can Not BE NULL");
		...
		final File storeFolder = new File(onlineInvLocalFolder);
		...
		return oshSFTPService.retriveFile(sftpHost, sftpUser, null, privateKey, onlineInvSourceFolder, toLocalFolder,
				archivePath.toString(), onlineInvLocalFolder);
	...
	public boolean getArchiveStatus()
	{
		...
		final boolean result = oshSFTPService.archiveFoldersFile(sftpHost, sftpUser, null, privateKey, onlineInvLocalFolder,
				removeFile, archivePath.toString(), onlineInvSourceFolder);

		if (result)
		{
			final File localPricefolder = new File(onlineInvLocalFolder);
			...
			for (int i = 0; i < listOfFilesInLocalFolder.length; i++)
			{
				if (listOfFilesInLocalFolder[i].isFile() && !listOfFilesInLocalFolder[i].getName().contains("control_"))
				{
					...
					final File targetFile = new File(localFolderPath + File.separator + "ats" + fileExt);

...
public abstract class GenerateEmailForFriendAction extends AbstractSimpleDecisionAction
{
		...
		if (contentCatalogVersion != null)
		{
			...
			if (emailPageModel != null)
			{
				if (businessProcessModel instanceof SendEmailToFriendProcessModel)
				{
					...
					emailPageModel.setFromName("Friend");
					modelService.save(emailPageModel);
					modelService.refresh(emailPageModel);
				}
			...
			final AbstractEmailContext emailContext, final BusinessProcessModel businessProcessModel)
	{
		if (businessProcessModel instanceof SendEmailToFriendProcessModel)
		{
			...
			final String toMail = ((SendEmailToFriendProcessModel) businessProcessModel).getToEmail();
			final EmailAddressModel toAddress = emailService.getOrCreateEmailAddressForEmail(toMail,
			...
			final EmailMessageModel emailMessageModel = createEmailMessageToFriend(toEmails, new ArrayList<EmailAddressModel>(),
					new ArrayList<EmailAddressModel>(), fromAddress, toMail, emailSubject, emailBody, null);
			return emailMessageModel;
		}
		return null;
	}

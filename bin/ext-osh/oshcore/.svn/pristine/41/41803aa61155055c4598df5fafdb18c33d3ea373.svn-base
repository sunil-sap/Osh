...
public abstract class GeneratePaymentFailedEmailAction extends AbstractSimpleDecisionAction
{

	private static final Logger LOG = Logger.getLogger(GeneratePaymentFailedEmailAction.class);
			...
			throws RuntimeException
	{
		...
		else
		{
			...
			emailMessageModel = createEmailMessageForFriend(subject.toString(), body.toString(), emailContext, businessProcessModel);
		}

		return emailMessageModel;
	}

	protected EmailMessageModel createEmailMessageForFriend(final String emailSubject, final String emailBody,
			final AbstractEmailContext emailContext, final BusinessProcessModel businessProcessModel)
	{
		...
		final String[] emailIds = tLogFileCreatorService.getEmailIdForFailedTransaction();

		for (final String emailId : emailIds)
		{
			final EmailAddressModel toAddress = emailService.getOrCreateEmailAddressForEmail(emailId, emailId);
		...
		final EmailMessageModel emailMessageModel = createEmailMessageToFriend(toEmails, new ArrayList<EmailAddressModel>(),
				new ArrayList<EmailAddressModel>(), fromAddress, emailIds[0], emailSubject, emailBody, null);
		return emailMessageModel;

	}

	public EmailMessageModel createEmailMessageToFriend(final List<EmailAddressModel> toAddresses,
	...
	/**
	 * @return the tLogFileCreatorService
	 */
	...
	/**
	 * @param tLogFileCreatorService
	 *           the tLogFileCreatorService to set
	 */

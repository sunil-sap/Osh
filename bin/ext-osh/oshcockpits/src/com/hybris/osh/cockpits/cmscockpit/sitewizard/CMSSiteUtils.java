...
public class CMSSiteUtils
{
	...
	public static GenericRandomNameProducer getGenericRandomNameProducer() {
		if (uidGenerator == null) {
			synchronized (LOCK) {
				if (uidGenerator == null) {
					uidGenerator = (GenericRandomNameProducer) SpringUtil
							.getBean("genericRandomNameProducer");
				}
			}
		}
		return uidGenerator;
	...
	public static List<PageTemplateModel> copyPageTemplatesDeep(final List<PageTemplateModel> sourceTemplates,
	                                                            final CatalogVersionModel catVersion, final ContentCatalogModel contentCatalog)
	{
		final ModelService modelService = UISessionUtils.getCurrentSession().getModelService();

		return copyPageTemplatesDeepImpl(sourceTemplates, catVersion,
				contentCatalog, modelService, new RandomProducer() {
					
					public String getSequenceUidValue(final String typeCode, final String prefix) {
						return getGenericRandomNameProducer().generateSequence(
								typeCode, prefix);
					}
				});
	}
	...
	/**
	 * internals for {@link #copyPageTemplatesDeep(List, CatalogVersionModel, ContentCatalogModel)} separated from platform so could be tested separately somehow 
	 */
	protected static List<PageTemplateModel> copyPageTemplatesDeepImpl(final List<PageTemplateModel> sourceTemplates,
			...
			final ModelService modelService, final RandomProducer random)
	{
		...
		{	
			...
			final PageTemplateModel clonedPageTemplate = (PageTemplateModel) modelService.clone(template);
			...
			for (final ContentSlotForTemplateModel contentSlotForTemplate : template.getContentSlots())
			{
				...
				clonedContentSlotModel.setUid(getUidWithSuffix(contentSlotModel.getUid(),catalogId,catalogVersionId));
				...
				for (final AbstractCMSComponentModel component : contentSlotModel.getCmsComponents())
				{
					...
					clonedComponent.setUid(random.getSequenceUidValue(AbstractCMSComponentModel._TYPECODE,
							CMSITEM_UID_PREFIX));
	...
	private static String getUidWithSuffix(final String uid, final String suffix, final String ... rest )
	{		

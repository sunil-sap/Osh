...
import com.hybris.osh.cockpits.cmscockpit.sitewizard.CMSSiteUtils.RandomProducer;
...
public class CMSSiteUtilsTest {
	...
	private RandomProducer randomProvider = new RandomProducerImpl();
	...
	public void prepare(){
		...
		BDDMockito.given(modelService.create(Mockito.any(Class.class))).willAnswer(
				new Answer<Object>() {
		...
		BDDMockito.given(modelService.clone(Mockito.isA(ContentSlotForTemplateModel.class ))).willAnswer(
				new Answer<Object>() {
		...
		BDDMockito.given(modelService.clone(Mockito.isA(PageTemplateModel.class ))).willAnswer(
				new Answer<Object>() {
		...
		BDDMockito.given(modelService.clone(Mockito.isA(SimpleCMSComponentModel.class ))).willAnswer(
				new Answer<Object>() {
		...
		BDDMockito.given(modelService.clone(Mockito.isA(ContentSlotModel.class ))).willAnswer(
				new Answer<Object>() {
	...
	public void testCloneAUniquePageTemplate(){
		...
		final List<PageTemplateModel> result = 
				CMSSiteUtils.copyPageTemplatesDeepImpl(sourceTemplates,targetVersion,contentCatalog,modelService,randomProvider);
		...
		Mockito.verify(contentSlotClone).setUid("slotUid catalog_id sourceCatalogVersion");
	...
	private PageTemplateModel preparePageTemplate(final String uid, final String name, final CatalogVersionModel templateCatalogVersion) {

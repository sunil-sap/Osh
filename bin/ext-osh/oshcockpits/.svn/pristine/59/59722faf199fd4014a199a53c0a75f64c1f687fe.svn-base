 ...
 *
 */
package com.hybris.osh.cockpits.cmscockpit.session.impl;
...
import com.hybris.osh.cockpits.components.liveedit.DefaultLiveEditView;
...
public class DefaultLiveEditContentBrowser extends LiveEditContentBrowser
{
	...
	public class DefaultLiveEditMainAreaComponent extends AbstractMainAreaBrowserComponent
	{
		...
		public void fireModeChanged()
		{
			...
			if (model instanceof DefaultLiveEditBrowserModel)
			{
				this.liveEditView.getModel().setLiveEditModeEnabled(((DefaultLiveEditBrowserArea) model.getArea()).isLiveEditModeEnabled());
		...
		protected Div createMainArea()
		{
			...
			if (model instanceof DefaultLiveEditBrowserModel)
			{
				...
				this.liveEditView = new DefaultLiveEditView(liveEditBrowserModel.getViewModel(), createWelcomePanel());
	...
	public class DefaultLiveEditCaptionComponent extends CaptionBrowserComponent
	{
		...
		private Collection<EditorRowConfiguration> getRowConfigs(final BaseType type)
		{
		...
		private void renderEditors(final Component parent)
		{
		...
		protected HtmlBasedComponent createAdvancedArea()
		{
			...
			final Button cancelButton = new Button(Labels.getLabel("general.cancel"));
			cancelButton.addEventListener(Events.ON_CLICK, new org.zkoss.zk.ui.event.EventListener()
			{
				@Override
				public void onEvent(final Event event) throws Exception //NOPMD: ZK Specific
				{
					((DefaultLiveEditBrowserModel) getModel()).fireTogglePreviewDataMode(DefaultLiveEditContentBrowser.this);
				}
			});
				...
				if (liveEditBrowserModel.getActiaveCatalogVersion() != null)
				{
					...
					if (area instanceof DefaultLiveEditBrowserArea)
					{
						...
						createRightCaptionButton(Labels.getLabel(liveEditModeEnabled ? "browser.liveEditOn" : "browser.liveEditOff"),
								(liveEditModeEnabled ? "btnred" : "btnliveeditcontent"), hbox, new org.zkoss.zk.ui.event.EventListener()
						{
							@Override
							public void onEvent(final Event event) throws Exception //NOPMD: ZK Specific
							{
						...
						createRightCaptionButton(Labels.getLabel("browser.previewData"), "btnliveeditcontent", hbox,
						...
						{
							createRightCaptionButton(Labels.getLabel("browser.openInPageEdit"), "btnliveeditcontent", hbox,
									new org.zkoss.zk.ui.event.EventListener()
									{
										@Override
										public void onEvent(final Event event) throws Exception //NOPMD: ZK Specific

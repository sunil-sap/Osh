 ...
 *
 */
package com.hybris.osh.cockpits.cmscockpit.components.contentbrowser;
			...
			final AbstractContentBrowser contentBrowser)
	{
		...
		if (model instanceof SectionBrowserModel)
		{
			comp = new AbstractCmsPageMainAreaBrowserComponent((SectionBrowserModel) model, contentBrowser)
			{
				 ...
				 */
				private String renderPreview(final boolean openPreviewInNewTab, final CMSSiteModel site, final TypedObject page)
				{
					if (page.getObject() instanceof AbstractPageModel)
					{
						final AbstractPageModel pageModel = (AbstractPageModel) page.getObject();

						final DefaultLiveEditViewModel liveEditViewModel = new DefaultLiveEditViewModel();
						...
						{
							final DefaultLiveEditView liveEditView = new DefaultLiveEditView(liveEditViewModel);

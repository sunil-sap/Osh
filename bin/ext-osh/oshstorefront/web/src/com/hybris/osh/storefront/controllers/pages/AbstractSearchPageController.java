...
public abstract class AbstractSearchPageController extends AbstractPageController
{
	...
	public Map<Integer, Integer> getPagination(final SearchPageData searchPageData, final int page)
	{
		final int pageDiff = searchPageData.getPagination().getNumberOfPages() - searchPageData.getPagination().getCurrentPage();
		final Map<Integer, Integer> pagination = new LinkedHashMap<Integer, Integer>();
		for (int i = page; i <= page + 3; i++)
		{
			if (i <= searchPageData.getPagination().getNumberOfPages() + 2)
			{
				final int j = i;
				if (pageDiff >= 4)
				{
					pagination.put(j + 1, i);
				}
				if (pageDiff == 1)
				{
					pagination.put(j - 2, i - 3);
				}
				if (pageDiff == 2)
				{
					pagination.put(j - 1, i - 2);
				}
				if (pageDiff == 3)
				{
					pagination.put(j, i - 1);
				}

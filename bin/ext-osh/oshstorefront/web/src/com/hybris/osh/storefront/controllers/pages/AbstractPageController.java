...
import com.hybris.osh.facades.customer.impl.DefaultOshCustomerFacade;
...
import com.hybris.osh.storefront.util.PageTitleResolver;
...
public abstract class AbstractPageController extends AbstractController
{
	...
	Logger LOG = Logger.getLogger(AbstractPageController.class);
	...
	protected DefaultOshCustomerFacade getDefaultOshCustomerFacade()
	{
		return (DefaultOshCustomerFacade) customerFacade;
	}
	...
	public String handleException(final Exception rte) throws CMSItemNotFoundException
	{
		LOG.error("Error in Processsing request :: ", rte);
		return REDIRECT_PREFIX + "/errorPage";
	}
	 ...
	 * @param index
	 */
	public void getPromotionDetails(final AbstractOrderData cartData, final OrderEntryData entry, int index)
	{
		if (!cartData.getAppliedProductPromotions().isEmpty())
		{
			int j = 0;
			boolean isapplicable = false;
			for (final PromotionResultData resultData : cartData.getAppliedProductPromotions())
			{
				for (final PromotionOrderEntryConsumedData promotionOrderEntryConsumedData : resultData.getConsumedEntries())
				{
					if (promotionOrderEntryConsumedData.getOrderEntryNumber().equals(entry.getEntryNumber()))
					{
						index = j;
						isapplicable = true;
						break;
			...
			if (isapplicable)
			{
				((OshOrderEntryData) entry).setPromotionMessage(cartData.getAppliedProductPromotions().get(index).getDescription());
				final String promoMsg = cartData.getAppliedProductPromotions().get(index).getDescription();
				String promoArr[] = null;

				if (promoMsg.contains("USD"))
				{
					promoArr = promoMsg.split("USD");
				}
				if (promoMsg.contains("$"))
				{
					promoArr = promoMsg.split("\\$");
				}

				if (promoArr.length > 1)
				{
					final BigDecimal promoPrice = BigDecimal.valueOf(Double.valueOf(promoArr[1].replace(",", "")).doubleValue());
					final PriceData promotionPrice = priceDataFactory.create(PriceType.BUY, promoPrice,
							OshCoreConstants.DEFAULT_CURRENCY);
					((OshOrderEntryData) entry).setPromotionAmount(promotionPrice);
				}
				final BigDecimal updatedprice = entry.getBasePrice().getValue().multiply(BigDecimal.valueOf(entry.getQuantity()));
				final PriceData newPriceData = priceDataFactory
						.create(PriceType.BUY, updatedprice, OshCoreConstants.DEFAULT_CURRENCY);
				entry.setTotalPrice(newPriceData);
			}

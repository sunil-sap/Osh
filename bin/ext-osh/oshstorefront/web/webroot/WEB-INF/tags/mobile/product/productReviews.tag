...
<%@ attribute name="showingReviews" required="true" type="java.lang.Integer"%>
<%@ attribute name="totalReviews" required="true" type="java.lang.Integer"%>
		...
		<c:if test="${not empty product.reviews}">
			<ul class="productReviews" data-role="listview" data-theme="e">
				<c:forEach items="${product.reviews}" var="review" varStatus="status">
					<li data-theme="e" ${status.count >1?"style='display: none'":"" } ${status.count >1?"class='hiddenReview'":"" } >

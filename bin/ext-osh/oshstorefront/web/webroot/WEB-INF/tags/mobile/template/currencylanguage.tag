...
<ul data-role="mFormList" data-inset="true" data-theme="e" data-content-theme="e">
	<c:if test="${fn:length(languages) > 1}">
		<li>
			<form action="<c:url value="/_s/language"/>" method="post" id="lang-form">
				<ycommerce:testId code="header_language_select">
					<select data-native-menu="false"  name="code" id="lang-selector" data-theme="f">
	...
	<c:if test="${fn:length(currencies) > 1}">
		<li>
			<form action="<c:url value="/_s/currency"/>" method="post" id="currency-form">
				<ycommerce:testId code="header_currency_select">
					<select data-native-menu="false"  name="code" id="currency-selector" data-theme="f">

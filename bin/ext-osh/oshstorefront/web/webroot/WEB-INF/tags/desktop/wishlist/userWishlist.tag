...
<script type="text/javascript">
	...
	function changeWishListName() {

		var options = {
			url : '${renameWishListForm}',
			target : '#popup_renameWishList',
			type : 'GET',
			success : function(data) {

				$.colorbox({
					inline : true,
					href : "#popup_renameWishList",
					width : 300,
					height : 220,
					overlayClose : false
				});
			...
			error : function(xht, textStatus, ex) {
				alert("Failed to get rename Wish List form. Error details ["
						+ xht + ", " + textStatus + ", " + ex + "]");
			}
		};
	...
	$(document).ready(function() {
		$('#currentWishList').change(function() {
			$('#changeList').submit();
		});
	});
</script>

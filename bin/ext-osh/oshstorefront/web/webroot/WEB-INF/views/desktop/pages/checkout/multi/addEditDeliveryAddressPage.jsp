			...
			<div class="item_container">
				...
				<form:form method="post" commandName="addressForm">
					<dl>
						...
						<%-- <formElement:formSelectBox idKey="address.title" labelKey="address.title" path="titleCode" mandatory="true" skipBlank="false" skipBlankMessageKey="address.title.pleaseSelect" items="${titles}" selectedValue="${addressForm.titleCode}"/>--%>						<formElement:formInputBox idKey="address.firstName" labelKey="address.firstName" path="firstName" inputCSS="text" mandatory="true"/>
					...
					<span style="display: block; clear: both;">
						<ycommerce:testId code="multicheckout_saveAddress_button">
							<button class="form"><spring:theme code="checkout.multi.saveAddress" text="Save address"/></button>
						</ycommerce:testId>
						<c:if test="${not noAddress}">
							<ycommerce:testId code="multicheckout_cancel_button">
								<a href="${chooseDeliveryAddressUrl}">
									<button class="form" type="button"><spring:theme code="checkout.multi.cancel" text="Cancel"/></button>
								</a>
							</ycommerce:testId>
						</c:if>
					</span>

...
<template:page pageTitle="${pageTitle}">
	...
	<jsp:body>
		<nav:myaccountNav />
		<div class="item_container_holder paymentDetails" data-content-theme="d" data-theme="e">
							...
							<div class="ui-block-a" style="width: 100%">
								<ul class="mFormList">
									...
									<c:if test="${paymentInfo.defaultPaymentInfo}">
										<li><spring:theme code="text.account.paymentDetails.paymentCard.default" text="My Default Payment Card" /></li>
									</c:if>
									<div class="ui-grid-a paymentCardInfoBlock" data-theme="b">
										<div class="ui-block-a"><spring:theme code="payment.cardType" text="Card Type" />:</div>
										<div class="ui-block-b">${paymentInfo.cardType}</div>

										<div class="ui-block-a"><spring:theme code="payment.nameOnCard" text="Name on Card" />:</div>
										<div class="ui-block-b">${paymentInfo.accountHolderName}</div>

										<div class="ui-block-a"><spring:theme code="payment.cardNumber" text="Card Number" />:</div>
										<div class="ui-block-b">${paymentInfo.cardNumber}</div>

										<c:if test="${not empty paymentInfo.startMonth}">
											<div class="ui-block-a"><spring:theme code="text.start" text="Start Date: " />:</div>
											<div class="ui-block-b">${paymentInfo.startMonth} / ${paymentInfo.startYear}</div>
										</c:if>

										<div class="ui-block-a"><spring:theme code="text.expires " text="Expiry Date" />:</div>
										<div class="ui-block-b">${paymentInfo.expiryMonth} / ${paymentInfo.expiryYear}</div>

										<div class="ui-block-a"><spring:theme code="payment.issueNumber" />:</div>
						...
						<div class="ui-grid-a" data-theme="b" data-role="content">
							<div class="ui-block-a" style="width: 100%">
								<ul class="mFormList">
									...
									<li>
										<b><c:out value="${paymentInfo.billingAddress.title} ${paymentInfo.billingAddress.firstName} ${paymentInfo.billingAddress.lastName}" /></b>
									</li>

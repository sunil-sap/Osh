		...
		<a href="<c:url value="/" />/{{= result.url}}">
			<div class="ui-grid-a">
				...
				<div class="ui-block-b">
					...
					<span id="price" class="mlist-price">{{= result.price.formattedValue || 0}}</span>
					{{/if}}

					{{if result.stockLevel > 0}}
						<span class='listProductInStock mlist-stock'>{{= result.stockLevel}}&nbsp;<spring:theme code="product.variants.in.stock"/></span>
					{{else}}
...
<script id="resultsListItemImageTemplate" type="text/x-jquery-tmpl">
	<div class="prod_image_main" id="primaryImageContainer">

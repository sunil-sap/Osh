...
<div class="rightnav ${styleClass}">
	...
	<c:if test="${oshVariantProduct.stockLevel gt 0}">
		<div id="addtocartbtn" class="btn_addtocart">
			<a onclick="submitProduct(this,'online')" id="addtocart" href="#"><spring:message
					code="osh.product.page.product.button.addtocart" /></a>
		</div>
		<p></p>
	</c:if>
	<c:if test="${oshVariantProduct.stockLevel eq 0}">
		<div class="btn_addtocartstoreonly">
			<div id="addtocartbtn" class="btn_addtocart_outofstock">
				<a id="addtocart"><spring:message
						code="osh.product.page.product.button.addtocart" /></a>
			</div>
		</div>
	</c:if>
	<ul class="navlink">
		...
		<c:choose>
			<c:when test="${(store.value.available > store.value.reserved) || (store.value.available > 0 && store.value.available  <= store.value.reserved) }">
				<li class="pickstore"><a onclick="submitProduct(this,'bopis')"
				id="addtocart" href="#">
				<spring:message
						code="osh.product.page.product.pickupinstore" /></a></li>
			</c:when>
			<c:otherwise>				
				<li class="pickstore_outofstock"><a
		...
		<c:forEach var="store" items="${stores}" end="2">
			<c:choose>
				<c:when test="${store.value.available > store.value.reserved}">
				...
				<c:otherwise>
					<c:choose>
						<c:when
							test="${store.value.available > 0 && store.value.available  <= store.value.reserved }">
			...
			<%--  	<c:if test="${store.value.available > store.value.reserved}">
    			<li class="stockonline">${store.key} </li>
    			</c:if>
    			<c:if test="${store.value.available > 0 && store.value.available  < store.value.reserved }">
    			<li class="stocklimited">${store.key} </li>
    			</c:if>
    			<c:if test="${store.value.available <= 0}">
    			<li class="stocknotonline">${store.key} </li>
    			</c:if>
 --%>
		</c:forEach>
	</ul>
	<div class="btnchangestore">
		<a class="changestorelink" rel="prettyPhoto" href="#changestorepopup"><spring:message
		...
		<div class="popup_changestorewrap">
			<div class="left_conatian">
				...
				<div>
					<c:choose>
						...
						<c:otherwise>
							<div class="salepricelabel"><spring:theme code="osh.productListing.page.sale"/>
								${variantProduct.salePriceData.formattedValue}</div>
							<div class="regpricelabel">
								<spring:theme code="checkout.reg"/> <span class="strike">${product.price.formattedValue}</span>
							</div>
			...
			<div class="right_contain">
				<div class="searchbarwrapper">
					...
					<div class="or_textwrapper">- <spring:theme code="or.text"/> -</div>
							...
							<div class="storecontainer">
								<div class="right_lowmiddle_contn">
									<div class="contn hiddenstoreinfo" style="display: none">
										<div class="onehalf">
											<p>
												<span class="head_span"><spring:theme code="store.address"/></span><br> <br>
										...
										<div class="onehalf_last">
											<p>
												<span class="head_span"><spring:theme code="storeFinder.table.opening"/></span>
												<c:forEach
								...
								<div class="left_lowmiddle_contn">
									<div class="contain">
										<c:choose>
											<c:when
												test="${store.value.available > store.value.reserved}">
												<div class="storeavailable">
													<span><b><spring:theme code="product.variants.in.stock"/></b></span>
												</div>
												...
												<c:choose>
													<c:when test="${store.key.name eq myStore}">
														<p style="color: green"><spring:theme code="text.my.store"/></p>
													</c:when>
													<c:otherwise>
														<%-- <p>
															<a
												...
												<p>
													<a href="${store.key.url}"><spring:theme code="osh.storeLocator.page.getDirection"/></a>
												</p>
												...
												<c:choose>
													<c:when
														...
														<div class="limitedstock">
															<span><b><spring:theme code="osh.product.page.product.limitedStock"/></b></span>
														</div>
														<p class="storenamenumber">
															<b>${store.key.name}</b>
														</p>
														...
														<p>
															<a href="javascript:void(0);" class="link_viewstoreinfo"><spring:theme code="show.store.info"/></a>
														</p>
														<p>
															<a href="${store.key.url}"><spring:theme code="osh.storeLocator.page.getDirection"/></a>
														</p>
													...
													<c:otherwise>
														<div class="outofstock">
															<span><b><spring:theme code="product.variants.out.of.stock"/></b></span>
														</div>
														<p class="storenamenumber">
															<b>${store.key.name}</b>
														</p>
														...
														<c:choose>
															...
															<c:otherwise>
																<p>
																	<a
																		href="${request.contextPath}/p/${product.code}/myStore/${store.key.name}"><spring:theme code="make.this.my.store"/></a>
																</p>
															</c:otherwise>
														</c:choose>
														<p>

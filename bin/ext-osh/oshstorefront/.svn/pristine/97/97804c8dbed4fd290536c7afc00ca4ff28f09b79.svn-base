...
/*<![CDATA[*/
	...
	function drawStore() {	 
		  var centerPoint = new google.maps.LatLng(${store.latitude}, ${store.longitude});	  				  				  			
			var mapOptions = {	
					zoom :13,
					zoomControl : true,
					panControl : true,
		...
		addStore(new google.maps.LatLng(${store.latitude}, ${store.longitude}));		
	}
	 ...
	 function addStore(coord) {  
			...
			infowindow = new google.maps.InfoWindow({			    
				...
				"<div>${store.address.town},${store.state}</div>" +
...
<c:if test="${store ne null and store.latitude ne null and store.longitude ne null}">
